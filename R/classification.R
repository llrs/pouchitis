library("pROC")
X<-read.table("16S_models.csv",sep=",",dec=".",header=T,row.names=1,check.names=F)
...
for( i in 1:dim(var)[2]) {
  ...
  plot.roc(flag,var[,i],main=colnames(var)[i],print.auc=T,ci=T)
...
########################################################################
####################################### rnaseq  ##################################
X<-read.table("RNASeq_models.csv",sep=",",dec=".",header=T,row.names=1,check.names=F)
